...
template <class real>
inline int memAlloc(real** ptr, size_t length) {
  void* ptr_;
  cudaMalloc(&ptr_, length * sizeof(real));
  *ptr = (real*)ptr_;
  if (ptr_ != NULL)
    return 0;
  else
...
template <class real>
inline int memAllocHost(real** ptr, real** ptr_dev, size_t length) {
  void* ptr_;
  cudaHostAlloc(&ptr_, length * sizeof(real), cudaHostAllocMapped);
...
template <class real>
inline int memAllocPitch(real** ptr, size_t lx, size_t ly, size_t* pitch) {
  int bl = lx * sizeof(real);
  void* ptr_;
  bl = iAlignUp(bl, 32);
  if (pitch) *pitch = bl / sizeof(real);
  int length = bl * ly;
