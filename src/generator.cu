...
void generate_problem_gpu(int m, int n, int nnz, FisherProblem &csr, double power = 1.0) {
    ...
    // 5. 构造 row_ptr：每行 nnz/m 个非零（动态分配）
    ...
    // 6. 构造 col_ind：生成 [0, n-1] 范围内的随机整数
    generate_random_integers<<<(nnz+255)/256, 256>>>(d_state, csr.col_ind, nnz, n);

    // 7. 确保每行的列索引是唯一的
    unique_columns<<<m, 1, 1024 * sizeof(int)>>>(csr.col_ind, csr.row_ptr, m, n);

    // 8. 计算 u_sum_dim_1 和 vec_val
    compute_u_sum_dim_1_and_vec_val<<<m, 1>>>(csr.u_val, csr.row_ptr, csr.b, d_u_sum_dim_1, d_vec_val, m);
    // 9. 计算 x0
